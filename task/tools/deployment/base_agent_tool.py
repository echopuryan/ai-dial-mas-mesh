            ...
            if chunk.choices and len(chunk.choices) > 0:
                ...
                if custom_content := delta.custom_content:
                    if custom_content.attachments:
                        custom_content.attachments.extend(custom_content.attachments)

                    if custom_content.state:
                        custom_content.state = custom_content.state

                    cc_dict = custom_content.dict(exclude_none=True)
    ...
    def _prepare_messages(self, tool_call_params: ToolCallParams) -> list[dict[str, Any]]:
        ...
        if propagate_history:
            for idx in range(len(tool_call_params.messages)):
                message = tool_call_params.messages[idx]
                if message.role == Role.ASSISTANT:
                    if message.custom_content and message.custom_content.state:
                        msg_state = message.custom_content.state
